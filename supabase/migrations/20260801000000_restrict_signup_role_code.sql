-- Self-signup must never be able to obtain a privileged role_code (대표/개발자),
-- even if the client bypasses the app UI and calls supabase.auth.signUp() directly.
-- Privileged roles can only be granted afterwards by an admin via
-- auth-policy's admin_update_user_profile action.
